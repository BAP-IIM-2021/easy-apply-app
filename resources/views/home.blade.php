<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>EasyApply</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://kit.fontawesome.com/2b02a79d23.js" crossorigin="anonymous"></script>

    <link href="style.css" rel="stylesheet">
</head>
<body class="bg-orange-50">

<header class="h-88 my-5">
    <nav class="flex justify-around">
        <div class="">
            <img class="w-3/12" src="./img/logo.png" alt="logo">
        </div>

        <div class="flex-col">
            <ul class="flex justify-center item-center">
                <li class="m-2">Accueil</li>
                <li class="flex m-2"><i class="fas fa-search"></i>Rechercher</li>
                <li class="block bg-cyan-400 rounded-lg p-2 m-2"><a href="">Se connecter</a></li>
            </ul>
        </div>
    </nav>
    <div class="bg-cyan-400 block text-white p-24 pb-48">
        <h1 class="text-5xl">Et si on postulait autrement ?</h1><br>
        <h3 class="text-3xl font-bold  py-2">Favorisons la rencontre entre l'offre et la demande d'emploi</h3>
        <p class="text-white">Optimisez votre recrutement à moindre coût grâce à notre service tout en un : publications illimitées d’offres, matching et scoring des</p>
        <p class="text-white">candidats, gestion automatisé des candidatures.</p>
    </div>
</header>

<article class="mt-36">
    <h2 class="text-center text-6xl mb-32"> Qui sommes-nous ?</h2>

    <div class="flex my-5 justify-evenly">
        <div class="ml-24">
            <iframe width="750" class="h-full"
                    src="https://www.youtube.com/watch?v=rcWzZz969qg&list=PLFHz2csJcgk8lgiRDB7FdsXVr4xy6jE8K&index=3">
            </iframe>
        </div>
        <div class="shadow-md bg-white rounded-lg ml-24 mr-24 p-10 relative">
            <div class="flex rounded-full bg-white shadow-md absolute left-1/3 -top-24 w-44 h-44 m-auto">
                <img width="170" height="170" class="m-auto" src="./img/logo.png" alt="logo">
            </div>

            <h3 class="uppercase text-center text-2xl font-bold mt-16">Easy Apply</h3>
            <p class="text-center">Application de recrutement pas système de matching</p>

            <h4 class="text-cyan-400 text-2xl mt-14">Compétences :</h4>
            <p class="mt-5"><span class="font-bold">Offres illimités :</span> les publications d'offres d'emploi sont illimitées</p>
            <p class="mt-5"><span class="font-bold">Visibilité de l'offre :</span> votre offre est proposée aux profils que vous cherchez et à ceux intéressés par vos mission</p>
            <p class="mt-5"><span class="font-bold">Gain de temps :</span> les candidats sont classées automatiquement selon vos critères.</p>
            <p class="mt-5"><span class="font-bold">Économies :</span> un tarif annuel et unique pour l'utilisation complète et illimitée de tous nos services</p>
        </div>
    </div>
</article>

<section class="flex mt-24">

    <div class="shadow-md bg-white rounded-lg p-10 m-5 relative">
        <div class="flex rounded-full bg-white shadow-md absolute left-1/3 -top-16 w-24 h-24 m-auto">
            <img width="100" height="100" class="m-auto" src="./img/logo.png" alt="logo">
        </div>

        <h3 class="uppercase text-center text-2xl font-bold mt-16">Easy Apply</h3>
        <p class="text-center">Application de recrutement pas système de matching</p>

        <h4 class="text-cyan-400 text-2xl mt-14">Compétences :</h4>
        <p class="mt-5"><span class="font-bold">Offres illimités :</span> les publications d'offres d'emploi sont illimitées</p>
        <p class="mt-5"><span class="font-bold">Visibilité de l'offre :</span> votre offre est proposée aux profils que vous cherchez et à ceux intéressés par vos mission</p>
        <p class="mt-5"><span class="font-bold">Gain de temps :</span> les candidats sont classées automatiquement selon vos critères.</p>
        <p class="mt-5"><span class="font-bold">Économies :</span> un tarif annuel et unique pour l'utilisation complète et illimitée de tous nos services</p>
    </div>

    <div class="shadow-md bg-white rounded-lg p-10 m-5 relative">
        <div class="flex rounded-full bg-white shadow-md absolute left-1/3 -top-16 w-24 h-24 m-auto">
            <img width="100" height="100" class="m-auto" src="./img/logo.png" alt="logo">
        </div>

        <h3 class="uppercase text-center text-2xl font-bold mt-16">Easy Apply</h3>
        <p class="text-center">Application de recrutement pas système de matching</p>

        <h4 class="text-cyan-400 text-2xl mt-14">Compétences :</h4>
        <p class="mt-5"><span class="font-bold">Offres illimités :</span> les publications d'offres d'emploi sont illimitées</p>
        <p class="mt-5"><span class="font-bold">Visibilité de l'offre :</span> votre offre est proposée aux profils que vous cherchez et à ceux intéressés par vos mission</p>
        <p class="mt-5"><span class="font-bold">Gain de temps :</span> les candidats sont classées automatiquement selon vos critères.</p>
        <p class="mt-5"><span class="font-bold">Économies :</span> un tarif annuel et unique pour l'utilisation complète et illimitée de tous nos services</p>
    </div>

    <div class="shadow-md bg-white rounded-lg p-10 m-5 relative">
        <div class="flex rounded-full bg-white shadow-md absolute left-1/3 -top-16 w-24 h-24 m-auto">
            <img width="100" height="100" class="m-auto" src="./img/logo.png" alt="logo">
        </div>

        <h3 class="uppercase text-center text-2xl font-bold mt-16">Easy Apply</h3>
        <p class="text-center">Application de recrutement pas système de matching</p>

        <h4 class="text-cyan-400 text-2xl mt-14">Compétences :</h4>
        <p class="mt-5"><span class="font-bold">Offres illimités :</span> les publications d'offres d'emploi sont illimitées</p>
        <p class="mt-5"><span class="font-bold">Visibilité de l'offre :</span> votre offre est proposée aux profils que vous cherchez et à ceux intéressés par vos mission</p>
        <p class="mt-5"><span class="font-bold">Gain de temps :</span> les candidats sont classées automatiquement selon vos critères.</p>
        <p class="mt-5"><span class="font-bold">Économies :</span> un tarif annuel et unique pour l'utilisation complète et illimitée de tous nos services</p>
    </div>

    <div class="shadow-md bg-white rounded-lg p-10 m-5 relative">
        <div class="flex rounded-full bg-white shadow-md absolute left-1/3 -top-16 w-24 h-24 m-auto">
            <img width="100" height="100" class="m-auto" src="./img/logo.png" alt="logo">
        </div>

        <h3 class="uppercase text-center text-2xl font-bold mt-16">Easy Apply</h3>
        <p class="text-center">Application de recrutement pas système de matching</p>

        <h4 class="text-cyan-400 text-2xl mt-14">Compétences :</h4>
        <p class="mt-5"><span class="font-bold">Offres illimités :</span> les publications d'offres d'emploi sont illimitées</p>
        <p class="mt-5"><span class="font-bold">Visibilité de l'offre :</span> votre offre est proposée aux profils que vous cherchez et à ceux intéressés par vos mission</p>
        <p class="mt-5"><span class="font-bold">Gain de temps :</span> les candidats sont classées automatiquement selon vos critères.</p>
        <p class="mt-5"><span class="font-bold">Économies :</span> un tarif annuel et unique pour l'utilisation complète et illimitée de tous nos services</p>
    </div>


</section>

<article class="bg-cyan-400 text-white m-36 p-8 h-80 mt-40">
    <h3 class="text-3xl mx-10 my-5">Vous aimeriez voir votre entreprise sur notre site ?</h3>
    <p class="font-bold text-2xl text-center mt-20">Créez votre page d'entreprise en appuyant simplement ici !</p><br>
    <p  class="text-center"><a href="" class="rounded-full bg-white text-black py-2 px-4 text-center">Page d'entreprise</a></p>
</article>

<footer class="bg-black text-white h-56">
    <div class="flex justify-evenly">
        <div class="">
            <img class="w-2/12" src="img/logo.png" alt="logo">
        </div>
        <div class="m-5">
            <h3 class="font-bold text-xl m-2">Nous contacter</h3>
            <p>project@easyapply.fr</p>
            <p>06.11.06.56.01</p>
        </div>
        <div class="m-5">
            <h3 class="font-bold text-xl m-2">S'inscrire</h3>
            <a href="">En temps qu'entreprise</a>
            <a href="">En temps que chercheur d'emploi</a>
        </div>
    </div>

    <div class="flex justify-center ">
        <i class="fab fa-instagram mx-5"></i>
        <i class="fab fa-linkedin mx-5"></i>
        <i class="fab fa-facebook-square mx-5"></i>
    </div>
</footer>

</body>
</html>